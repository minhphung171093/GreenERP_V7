...
class Parser(report_sxw.rml_parse):
    ...
    def get_lines(self):
        ...
        res = self.get_lines_theongay(self.cr, product[0], date_from, date_to)
        return res
    ...
    def get_lines_theongay(self, cr, product_id, date_from, date_to):
        res = []
        seq = -1
        sql = '''
            ...
            sql = '''
                select case when sum(sl_trung)!=0 then sum(sl_trung) else 0 end sl_trung,
                    ...
                    from tra_thuong_thucte_line where loai='2_so' and giai='dau' and product_id=%s
                        and trathuong_id in (select id from tra_thuong_thucte where ngay_tra_thuong between '%s' and '%s'
                        and daily_id=%s and state='done') 
            '''%(product_id, date_from, date_to, line['daily_id'])
            cr.execute(sql)
