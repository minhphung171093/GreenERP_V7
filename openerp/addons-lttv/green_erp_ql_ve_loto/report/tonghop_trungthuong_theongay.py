        ...
        for menhgia in menhgia_obj.browse(self.cr,self.uid,menhgia_ids):
            ...
            for line in loto_line_obj.browse(self.cr, self.uid, loto_line_ids):
                ...
                if line.sl_4_16_trung:
                    ...
                    tong_thanhtien += thanhtien
        res = {
            'tong_sl': format(tong_sl,',').split('.')[0].replace(',','.'),
            'tong_sl_trung': format(tong_sl_trung,',').split('.')[0].replace(',','.'),
            'tong_thanhtien': format(tong_thanhtien,',').split('.')[0].replace(',','.'),
        }
        return res
    
    def get_chitiet(self,menhgia):
        # 2 so
        tong_sl_2_d = 0
        tong_sl_2_d_trung = 0
        tong_tien_2_d = 0
        ...
        res = {
            ...
            'tong_sl_4_16_trung': format(tong_sl_4_16_trung, ',').split('.')[0].replace(',','.'),
