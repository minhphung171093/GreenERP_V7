...
class Parser(report_sxw.rml_parse):
    ...
    def get_lines(self):
        ...
        daily = wizard_data['daily_id']
        date_from = wizard_data['date_from']
        date_to = wizard_data['date_to']
        res = self.get_lines_theongay(self.cr, product[0], daily[0], date_from, date_to)
        return res
    ...
    def get_daily(self):
        wizard_data = self.localcontext['data']['form']
        daily = wizard_data['daily_id']
        partner = self.pool.get('res.partner').browse(self.cr, self.uid, daily[0])
        return partner.name
    ...
    def get_lines_theongay(self, cr, product_id, daily_id, date_from, date_to):
        res = []
        seq = -1
        sql = '''
            select tttt.ngay_tra_thuong as ngay_tra_thuong
                from tra_thuong_thucte tttt
                where tttt.daily_id = %s and tttt.state='done' and tttt.ngay_tra_thuong between '%s' and '%s' and tttt.id in (select trathuong_id from tra_thuong_thucte_line where product_id=%s)
                group by tttt.ngay_tra_thuong
                order by tttt.ngay_tra_thuong
        '''%(daily_id, date_from, date_to,product_id)
        cr.execute(sql)
        for seq_tt,line in enumerate(cr.dictfetchall()):
            seq += 1
            seq_n = seq
            res.append({
                'ngay_tra_thuong': line['ngay_tra_thuong'],
            ...
            sql = '''
                select case when sum(sl_trung)!=0 then sum(sl_trung) else 0 end sl_trung,
                    ...
                    from tra_thuong_thucte_line where loai='2_so' and giai='dau' and product_id=%s
                        and trathuong_id in (select id from tra_thuong_thucte where ngay_tra_thuong = '%s' and daily_id=%s and state='done') 
            '''%(product_id, line['ngay_tra_thuong'], daily_id)
            cr.execute(sql)
