    ...
    def get_line_by_product(self):
        ...
        for line in menh_gia_ids:
            sql = '''
                    ...
                    from
                        (select product_id,(select product_qty from stock_move where product_id = %s and date < '%s' and picking_id in (select id from stock_picking where type='in')) dau_ky,
                        (select product_qty from stock_move where product_id = %s and date between '%s' and '%s' and picking_id in (select id from stock_picking where type='in')) nhan_trong_ky,
                        (select product_qty from stock_move where product_id = %s and date between '%s' and '%s' and picking_id in (select id from stock_picking where type='out')) xuat_trong_ky,
                        (select product_qty from stock_move where product_id = %s and date < '%s' and picking_id in (select id from stock_picking where type='in')) cuoi_ky
                            from stock_move where product_id = %s
                        union all
                        select product_id,(select product_qty*-1 from stock_move where product_id = %s and date < '%s' and picking_id in (select id from stock_picking where type='out')) dau_ky,
                        0 as nhan_trong_ky,
                        0 as xuat_trong_ky,
                        (select product_qty from stock_move where product_id = %s and date < '%s' and picking_id in (select id from stock_picking where type='out')) cuoi_ky
