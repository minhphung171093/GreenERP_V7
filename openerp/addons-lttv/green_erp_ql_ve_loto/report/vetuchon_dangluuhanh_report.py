        ...
        for line in menh_gia_ids:
            ...
            sql='''
                select name, ((select case when sum(product_qty)!=0 then sum(product_qty) else 0 end sl from stock_move where picking_id in (select id from stock_picking where type='out' and ky_ve_id=%(ky_ve_id)s) and product_id = %(product_id)s
                        and date(timezone('UTC',date))<'%(date_from)s' and partner_id in %(dl_ids)s and state='done') - (select case when sum(tong_cong)!=0 then sum(tong_cong) else 0 end sl from ve_loto where product_id=%(product_id)s and ky_ve_id=%(ky_ve_id)s and ngay<'%(date_from)s' and state='done' and daily_id in %(dl_ids)s)
                                    ...
                                    - (select case when sum(tong_sai_kythuat)!=0 then sum(tong_sai_kythuat) else 0 end sl from ve_loto where product_id=%(product_id)s and ky_ve_id=%(ky_ve_id)s and ngay<'%(date_from)s' and state='done' and daily_id in %(dl_ids)s)) dau_ky,
                        ...
                        (select case when sum(tong_cong)!=0 then sum(tong_cong) else 0 end sl from ve_loto where product_id=%(product_id)s and ky_ve_id=%(ky_ve_id)s and ngay between '%(date_from)s' and '%(date_to)s' and state='done' and daily_id in %(dl_ids)s) ban_trg_ky, 
                        (select case when sum(tong_sai_kythuat)!=0 then sum(tong_sai_kythuat) else 0 end sl from ve_loto where product_id=%(product_id)s and ky_ve_id=%(ky_ve_id)s and ngay between '%(date_from)s' and '%(date_to)s' and state='done' and daily_id in %(dl_ids)s) sai_trg_ky,
                        ...
                        ((select case when sum(product_qty)!=0 then sum(product_qty) else 0 end sl from stock_move where picking_id in (select id from stock_picking where type='out' and ky_ve_id=%(ky_ve_id)s) and product_id = %(product_id)s
                        and date(timezone('UTC',date))<='%(date_to)s' and partner_id in %(dl_ids)s and state='done') - (select case when sum(tong_cong)!=0 then sum(tong_cong) else 0 end sl from ve_loto where product_id=%(product_id)s and ky_ve_id=%(ky_ve_id)s and ngay<='%(date_to)s' and state='done' and daily_id in %(dl_ids)s)
                                    ...
                                    - (select case when sum(tong_sai_kythuat)!=0 then sum(tong_sai_kythuat) else 0 end sl from ve_loto where product_id=%(product_id)s and ky_ve_id=%(ky_ve_id)s and ngay<='%(date_to)s' and state='done' and daily_id in %(dl_ids)s)) luu_hanh_cuoi_ky,
                        (((select case when sum(product_qty)!=0 then sum(product_qty) else 0 end sl from stock_move where picking_id in (select id from stock_picking where type='out' and ky_ve_id=%(ky_ve_id)s) and product_id = %(product_id)s
                        and date(timezone('UTC',date))<='%(date_to)s' and partner_id in %(dl_ids)s) - (select case when sum(tong_cong)!=0 then sum(tong_cong) else 0 end sl from ve_loto where product_id=%(product_id)s and ky_ve_id=%(ky_ve_id)s and ngay<='%(date_to)s' and state='done' and daily_id in %(dl_ids)s)
                                    - (select case when sum(sl)!=0 then sum(sl) else 0 end sl from ve_e_line where product_id=%(product_id)s and ve_e_id in (select id from ve_e where ngay between '%(date_from)s' and '%(date_to)s' and ky_ve_id=%(ky_ve_id)s and state='done' and name in %(dl_ids)s))
                                    - (select case when sum(tong_sai_kythuat)!=0 then sum(tong_sai_kythuat) else 0 end sl from ve_loto where product_id=%(product_id)s and ky_ve_id=%(ky_ve_id)s and ngay<='%(date_to)s' and state='done' and daily_id in %(dl_ids)s))*list_price::int) tien_cuoi_ky
