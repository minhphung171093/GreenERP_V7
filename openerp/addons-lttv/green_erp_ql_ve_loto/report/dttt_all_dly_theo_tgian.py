    ...
    def get_datas(self):
        ...
        sql = '''
            select lt.product_id as product_id, lt.daily_id as daily_id, pt.list_price as list_price, rp.parent_id as daily_cha_id,
                ...
                where lt.state='done' and lt.ngay between '%s' and '%s'
        ...
        for line in self.cr.dictfetchall():
            ...
            if self.data_dict.get(line['daily_id'], False):
                if self.data_dict[line['daily_id']].get(line['product_id'], False):
                    self.data_dict[line['daily_id']][line['product_id']]['sl_2'] += sluong_2
                    self.data_dict[line['daily_id']][line['product_id']]['sl_3'] += sluong_3
                    self.data_dict[line['daily_id']][line['product_id']]['sl_4'] += sluong_4
                    self.data_dict[line['daily_id']][line['product_id']]['tong_ve'] += tongve
                    self.data_dict[line['daily_id']][line['product_id']]['thanhtien'] += thanhtien
                    self.data_dict[line['daily_id']][line['product_id']]['tong_sai_kythuat'] += tong_sai_kythuat
                else:
                    self.data_dict[line['daily_id']][line['product_id']] = {
                            'sl_2': sluong_2,
                            'sl_3': sluong_3,
                            'sl_4': sluong_4,
                            'tong_ve': tongve,
...
#                 }
    ...
    def get_line_tong_all(self):
        return {
                ...
                'tong_ve': format(self.data_tong_all_dict['tong_ve'],',').split('.')[0].replace(',','.'),
