...
class Parser(report_sxw.rml_parse):
    ...
    def get_daily(self):
        ...
        dai_ly_ids = dai_ly_obj.search(self.cr ,self.uid, [('dai_ly','=',True)])
        return dai_ly_obj.browse(self.cr, self.uid, dai_ly_ids)
    
    def get_tong(self):
        rs = [{'tong_sluong_trung': format(self.total_sluong,',').split('.')[0].replace(',','.'),
               'tong_slan_trung': format(self.total_slan,',').split('.')[0].replace(',','.'),
               'tong_thanhtien': format(self.total_thanhtien,',').split('.')[0].replace(',','.'),
               }]
        ...
        for menhgia in product_product_ids:
            ...
            if kq:
                ...
                res.append(({
                    ...
                    'tong_sl_2_d': format(kq['tong_sl_2_d'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_2_d_trung': format(kq['tong_sl_2_d_trung'], ',').split('.')[0].replace(',','.'),
                    'tong_tien_2_d': format(kq['tong_tien_2_d'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_2_c': format(kq['tong_sl_2_c'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_2_c_trung': format(kq['tong_sl_2_c_trung'], ',').split('.')[0].replace(',','.'),
                    'tong_tien_2_c': format(kq['tong_tien_2_c'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_2_dc': format(kq['tong_sl_2_dc'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_2_dc_trung': format(kq['tong_sl_2_dc_trung'], ',').split('.')[0].replace(',','.'),
                    'tong_tien_2_dc': format(kq['tong_tien_2_dc'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_2_18': format(kq['tong_sl_2_18'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_2_18_trung': format(kq['tong_sl_2_18_trung'], ',').split('.')[0].replace(',','.'),
                    'tong_tien_2_18': format(kq['tong_tien_2_18'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_2': format(tong_sl_2, ',').split('.')[0].replace(',','.'),
                    'tong_sl_2_trung': format(tong_sl_2_trung, ',').split('.')[0].replace(',','.'),
                    'tong_tien_2': format(tong_tien_2, ',').split('.')[0].replace(',','.'),
                    'tong_sl_3_d': format(kq['tong_sl_3_d'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_3_d_trung': format(kq['tong_sl_3_d_trung'], ',').split('.')[0].replace(',','.'),
                    'tong_tien_3_d': format(kq['tong_tien_3_d'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_3_c': format(kq['tong_sl_3_c'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_3_c_trung': format(kq['tong_sl_3_c_trung'], ',').split('.')[0].replace(',','.'),
                    'tong_tien_3_c': format(kq['tong_tien_3_c'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_3_dc': format(kq['tong_sl_3_dc'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_3_dc_trung': format(kq['tong_sl_3_dc_trung'], ',').split('.')[0].replace(',','.'),
                    'tong_tien_3_dc': format(kq['tong_tien_3_dc'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_3_7': format(kq['tong_sl_3_7'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_3_7_trung': format(kq['tong_sl_3_7_trung'], ',').split('.')[0].replace(',','.'),
                    'tong_tien_3_7': format(kq['tong_tien_3_7'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_3_17': format(kq['tong_sl_3_17'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_3_17_trung': format(kq['tong_sl_3_17_trung'], ',').split('.')[0].replace(',','.'),
                    'tong_tien_3_17': format(kq['tong_tien_3_17'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_3': format(tong_sl_3, ',').split('.')[0].replace(',','.'),
                    'tong_sl_3_trung': format(tong_sl_3_trung, ',').split('.')[0].replace(',','.'),
                    'tong_tien_3': format(tong_tien_3, ',').split('.')[0].replace(',','.'),
                    'tong_sl_4_16': format(kq['tong_sl_4_16'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_4_16_trung': format(kq['tong_sl_4_16_trung'], ',').split('.')[0].replace(',','.'),
                    'tong_tien_4_16': format(kq['tong_tien_4_16'], ',').split('.')[0].replace(',','.'),
                    'tong_sl_4': format(tong_sl_4, ',').split('.')[0].replace(',','.'),
                    'tong_sl_4_trung': format(tong_sl_4_trung, ',').split('.')[0].replace(',','.'),
