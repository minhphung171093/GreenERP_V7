...
class Parser(report_sxw.rml_parse):
    ...
    def get_line(self):
        ...
        for line in invoice_obj.browse(self.cr,self.uid,self.ids):
            sql='''
            select partner_id,residual from account_invoice where state='open'and partner_id in (select id from res_partner where user_id=15)

select id from account_invoice_line where product_id in (select id from product_product where product_tmpl_id in (select id from product_template where categ_id in(select id from product_category where code='VC' ))) and invoice_id in ()

            '''
        sql='''
            select partner_id,residual from account_invoice where state='open'and partner_id in (select id from res_partner where user_id=%s)
        '''%(user_id)
        self.cr.execute(sql)
