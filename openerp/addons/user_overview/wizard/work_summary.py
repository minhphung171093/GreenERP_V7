...
class work_summary(osv.osv_memory):
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        res.update({
                    'previous_assigned_to': task.project_id.user_id.id,
                    'checking_task': task.checking_task,
                    'work_ids':[(0,0,{
                                      'name': 'Summary ...',
    ...
    def save(self, cr, uid, ids, context=None):
        ...
        effective_hours = task_obj.browse(cr, uid, task_id).effective_hours
        task_obj.write(cr, uid, [task_id], {'hours_spent_now':effective_hours})
        if line.is_done == True:
            if line.previous_assigned_to:
                ...
                msg_id = user_obj.message_post(cr, uid, [line.previous_assigned_to.id], type='comment', subtype=False, context=context, **post_values)
            ...
            task_type_id = task_type_obj.search(cr, uid, [('name','=','Delivery')])
