...
class user_overview(osv.osv):
    ...
    def _attendances_times(self, cr, uid, context=None):
        ...
        list_attendances_time =  attendances_times_obj.search(cr,1,[('day','=',date_now),('user_id','=',uid)])
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        if not total_id:
            ...
            if total_date != date_now:
                total_obj.write(cr, uid, total_id, {
                                    'name': '0:00:00',
                                    })
...
class project_task(osv.osv):
    ...
    def _planned_hours_user_get(self, cr, uid, ids, field_names, args, context=None):
        res = {}
        for task in self.browse(cr, 1, ids, context=context):
    ...
    _columns = {
             ...
             'hours_spent_now': fields.float('Hours Spent', digits=(16,2), readonly=True),
             'time_start': fields.datetime('Time Start', select=1, readonly=True),
...
#                                                    'attendance_id' : context['active_id'],
            ...
            if not task_id.date_start:
                self.write(cr,uid,ids,{'date_start':time.strftime('%Y-%m-%d %H:%M:%S')})
        self.write(cr,uid,ids,{'flag':False,'status_task': 'working','time_start': time_start})
        current_status_obj = self.pool.get('current.status')
    ...
    def logout_all_task(self, cr, uid, context=None):
        task_ids = self.search(cr,uid,[('stage_id.state','=','open'),('flag','=',False)])
        user_overview_obj = self.pool.get('user.overview')
        attendances_time_obj = self.pool.get('attendances.time')
        if task_ids:
        ...
        for user in user_ids:
            hr_sign_in = []
            hr_sign_out = []
            hr_attendance_ids = hr_attendance_obj.search(cr, 1, [('day','=',date_now),('employee_id.user_id','=',user)])
            if hr_attendance_ids:
            ...
            user_overview_obj.onchange_attendances_times(cr, user, context=context)
        return True
    ...
    def attendance_update(self, cr, uid, context=None):
        ...
        user_overview_obj = self.pool.get('user.overview')
        working_summary_obj = self.pool.get('working.summary')
        working_summary_id = working_summary_obj.search(cr,uid,[('date','=',date_now)])
        for working_summary in working_summary_obj.browse(cr, uid, working_summary_id):
    ...
    def update_time_spent(self, cr, uid, context=None):
        date_now_d = datetime.datetime.now() + timedelta(hours=7)
        task_ids = self.search(cr, uid, [('status_task','=','working')])
        for task in self.browse(cr, uid, task_ids):
            hours_spent = task.effective_hours
            time_start = datetime.datetime.strptime(task.time_start, '%Y-%m-%d %H:%M:%S') + timedelta(hours=7)
            hours = float(self.pool.get('user.overview')._sum_total(cr, uid, uid, time_start, date_now_d))/3600
            self.write(cr, uid, [task.id], {'hours_spent_now': hours+hours_spent})
        res = {
               'type': 'ir.actions.client',
               'tag': 'reload',
               'context' : context,
