...
class Parser(report_sxw.rml_parse):
    ...
    def show_dauky(self):
        ...
        self.cr.execute(sql)
        dk = self.cr.dictfetchall()
        for l in dk:
            self.dauky = {'debit':l['dr_amount'],'credit':l['cr_amount']}
        return dk
                ...
                if line['sum_cr'] == i['sum_dr']:    
                    ...
                    SELECT  am.date gl_date, coalesce(am.date_document,am.date) doc_date, am.name doc_no, 
                            coalesce(aih.product_showin_report, coalesce(avh.dien_giai,
                                concat(am.ref, am.ref_number))) description,acc.code acc_code,rp.name as tenkh,rp.internal_code as makh,                 
                    ...
                    for j in self.cr.dictfetchall():
                        if j['credit']:
                            if self.dauky['credit']-j['credit']<=0:
                                self.dauky['debit']=self.dauky['debit']+(j['credit']-self.dauky['credit'])
                                self.dauky['credit'] = 0
                            else:
                                self.dauky['credit']=self.dauky['credit']-j['credit']
                                self.dauky['debit'] = 0
                                
                        if j['debit']:
                            if self.dauky['debit']-j['debit']<=0:
                                self.dauky['credit']=self.dauky['credit']+(j['debit']-self.dauky['debit'])
                                self.dauky['debit'] = 0
                            else:
                                self.dauky['debit']=self.dauky['debit']-j['debit']
                                self.dauky['credit'] = 0
                        ...
                        res.append({
                                     ...
                                     'du_debit': self.dauky['debit'],
                                     'du_credit': self.dauky['credit'],
                                     'makh':j['makh'],
                                     'tenkh':j['tenkh'],
                                 })
                else:
                    ...
                    sql='''
                        select row_number() over(order by am.date, am.date_document, am.name)::int seq, 
                            ...
                            coalesce(aih.product_showin_report, coalesce(avh.dien_giai,
                                concat(am.ref, am.ref_number))) description,rp.name as tenkh,rp.internal_code as makh,
                    ...
                    for j in self.cr.dictfetchall():
                        if j['debit']:
                            if self.dauky['credit']-j['debit']<=0:
                                self.dauky['debit']=self.dauky['debit']+(j['debit']-self.dauky['credit'])
                                self.dauky['credit'] = 0
                            else:
                                self.dauky['credit']=self.dauky['credit']-j['debit']
                                self.dauky['debit'] = 0
                                
                        if j['credit']:
                            if self.dauky['debit']-j['credit']<=0:
                                self.dauky['credit']=self.dauky['credit']+(j['credit']-self.dauky['debit'])
                                self.dauky['debit'] = 0
                            else:
                                self.dauky['debit']=self.dauky['debit']-j['credit']
                                self.dauky['credit'] = 0
