...
class Parser(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({
            ...
            'get_no_line_tax':self.get_no_line_tax,
    ...
    def get_tax(self):
        ...
        if tax_id:
            tax = self.pool.get('account.tax').browse(self.cr,self.uid,tax_id[0])
            return tax.amount
        else:
            return ''
    ...
    def get_no_line_tax(self):
        res = []
        self.amount = 0
        self.amount_tax = 0
        if self.shop_ids:
            sql='''
                 SELECT agi.reference,agi.reference_number,agi.date_invoice,rp.name partner_name,rp.vat vat_code,agi.amount_untaxed,agi.amount_tax,agi.type,
                 pt.name as product,
                ...
                inner join product_product pp on pp.id = agil.product_id 
                inner join product_template pt on pt.id = pp.product_tmpl_id 
                WHERE agi.state in ('open','paid')
                AND agil.id not in (select invoice_line_id from account_invoice_line_tax)
                AND agi.date_invoice between '%s' and '%s'
                AND agi.shop_id in (%s)
                AND agi.type in ('out_invoice','out_refund')
                GROUP BY agi.reference,agi.reference_number,agi.date_invoice,rp.name,rp.vat,agi.amount_untaxed,agi.amount_tax,agi.type,pt.name
...
#                  'price_subtotal': line['type']=='out_refund' and -line['amount_untaxed'] or line['amount_untaxed'],
                 'price_subtotal': line['price_subtotal'],
#                  'amount_tax': line['type']=='out_refund' and -line['amount_tax'] or line['amount_tax'],
                 'amount_tax': 0,
                 'tax': '',
    ...
    def get_line_tax_0(self):
        ...
        if self.shop_ids:
            sql='''
                 SELECT agi.reference,agi.reference_number,agi.date_invoice,rp.name partner_name,rp.vat vat_code,agi.amount_untaxed,agi.amount_tax,agi.type,
                 atx.amount*100 as tax,pt.name as product,
                 SUM(CASE WHEN agi.type ='out_invoice' then agil.price_subtotal else agil.price_subtotal * (-1) end) as price_subtotal,                        
                 SUM(CASE WHEN agi.type ='out_invoice' then agil.price_subtotal *0 else agil.price_subtotal *0 end) as price_tax  
                FROM account_invoice agi 
                inner join account_invoice_line agil on agi.id = agil.invoice_id 
                inner join res_partner rp on rp.id = agi.partner_id  
                ...
                AND agi.shop_id in (%s)
                AND agi.type in ('out_invoice','out_refund')
                GROUP BY agi.reference,agi.reference_number,agi.date_invoice,rp.name,rp.vat,agi.amount_untaxed,agi.amount_tax,agi.type,atx.amount,pt.name
                ORDER BY date_invoice
            '''%(self.start_date,self.end_date,self.shop_ids)
        else:
            sql='''
...
#                  'amount_tax': line['type']=='out_refund' and -line['amount_tax'] or line['amount_tax'],
                 ...
                 'tax': line['tax'] and str(int(line['tax']))+'%' or '',
                 'product': line['product'],
                 })
            self.amount += line['price_subtotal']
    ...
    def get_line_tax_10(self):
        ...
        if self.shop_ids:
            sql='''
                    ...
                    atx.amount*100 as tax,pt.name as product,
                     SUM(CASE WHEN agi.type ='out_invoice' then agil.price_subtotal else agil.price_subtotal * (-1) end) as price_subtotal,                        
                     SUM(CASE WHEN agi.type ='out_invoice' then agil.price_subtotal *0.1 else agil.price_subtotal *0.1 * (-1) end) as price_tax  
                    FROM account_invoice agi 
                    inner join account_invoice_line agil on agi.id = agil.invoice_id 
                    inner join res_partner rp on rp.id = agi.partner_id  
                    inner join account_invoice_line_tax agilt on agil.id = agilt.invoice_line_id
