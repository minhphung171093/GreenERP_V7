...
class stock_balancesheet_account_report(osv.osv_memory):
    _name = "stock.balancesheet.account.report"
    ...
    _columns = {
        'short_by':fields.selection([
            (1 , 'Theo danh mục'),
            (2 , 'Theo sản phẩm')], 'Nhóm theo',required=True),
        'date_start':fields.date('Ngày bắt đầu',required=True),
        'date_end':fields.date('Đến ngày',required=True),
        'shop_ids': fields.many2many('sale.shop', 'balancesheet_shop_account_rel', 'balanceshe_acc_id', 'shop_id', 'Chi nhánh',required=True),
        'categ_ids': fields.many2many('product.category', 'balancesheet_category_account_rel', 'balanceshe_acc_id', 'categ_id', 'Danh mục',domain=[('type','<>','view')]),
        'location_id':fields.many2one('stock.location','Vị trí kho',domain=[('usage','<>','view')]),
     }
    _defaults = {
        'date_start':time.strftime('%Y-%m-01'),
        'short_by':2,
    ...
    def stock_report(self, cr, uid, ids, context=None): 
        datas = {'ids': context.get('active_ids', [])}
        datas['model'] = 'stock.balancesheet.account.report'
        datas['form'] = self.read(cr, uid, ids)[0]
        short_by = datas['form'] and datas['form']['short_by']
        if short_by and short_by==1:
            report_name = 'stock_balancesheet_categ_account_report'
        else:
            report_name = 'stock_balancesheet_product_account_report'
            
        return {'type': 'ir.actions.report.xml', 'report_name': report_name , 'datas': datas}
     
stock_balancesheet_account_report()
