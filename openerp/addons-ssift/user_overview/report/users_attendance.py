...
def emp_create_xml(cr, id, som, eom, emp):
    ...
    cr.execute(
        "select w.day as date, date_part('epoch', total_attendance::interval)/28800 as amount "\
        "from working_summary w "\
        "where w.user_id=%s and w.day >= %s and w.day < %s "
        "order by w.day",
...
report_custom('report.hr.analytical.attendance_users', 'hr.employee', '', 'addons/user_overview/report/users_attendance.xsl')
