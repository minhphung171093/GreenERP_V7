...
#             time_logout = None
    ...
    _columns = {
             'tasks': fields.one2many('project.task', 'task_id', "My Tasks", domain=[('stage_id.state','not in',['done','cancelled']),('checking_task','=','none')],readonly=False),
             'checking_tasks1': fields.one2many('project.task', 'task_id', "My Checking", domain=[('stage_id.state','not in',['done','cancelled']),('checking_task','=','checking1')],readonly=False),
             'checking_tasks2': fields.one2many('project.task', 'task_id', "My Checking", domain=[('stage_id.state','not in',['done','cancelled']),('checking_task','=','checking2')],readonly=False),
...
class project(osv.osv):
    ...
    def onchange_type_ids(self, cr, uid, context=None):
        if context:
            ...
            project_id = None
        if project_id:
            project = self.pool.get('project.project').browse(cr, 1, project_id)
...
class project_task(osv.osv):
    ...
    _columns = {
             ...
             'past_stage_id': fields.many2one('project.task.type','Past Stage', readonly=True),
             'checking_task': fields.selection([('checking1','checking1'),('checking2','checking2'),('none','None')], 'Status', readonly=False),
    ...
    def task_login(self, cr, uid, ids, context=None):
        ...
        attendances_time_obj = self.pool.get('attendances.time')
        check_obj = self.pool.get('check.login.logout')
        check_id = check_obj.search(cr,uid,[('user_id','=',uid)])
        line = check_obj.browse(cr,uid,check_id)[0]
        if line.check==False:
            raise osv.except_osv(_('Warning!'),
...
#                                                    'attendance_id' : context['active_id'],
        ...
        self.write(cr,uid,ids,{'flag':False,'status_task': 'working'})
        current_status_obj = self.pool.get('current.status')
        current_status_id = current_status_obj.search(cr,uid,[('user_id','=',uid)])
        current_status_obj.write(cr,uid,current_status_id,{'name':'working'})
        check_obj.write(cr,uid,check_id,{'check':False})
        user_overview_obj = self.pool.get('user.overview')
    ...
    def task_logout(self, cr, uid, ids, context=None):
        ...
        check_obj.write(cr,uid,check_id,{'check':True})
        return {
            'view_type': 'form',
            'view_mode': 'form',
    ...
    def quick_check(self, cr, uid, ids, context=None):
        obj_model = self.pool.get('ir.model.data')
        model_data_ids = obj_model.search(cr,uid,[('model','=','ir.ui.view'),('name','=','work_summary_view_form')])
        resource_id = obj_model.read(cr, uid, model_data_ids, fields=['res_id'])[0]['res_id']
        context.update({'quick_check':True})
        return {
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'work.summary',
    ...
    def logout_all_task(self, cr, uid, context=None):
        ...
        attendances_time_obj = self.pool.get('attendances.time')
        if task_ids:
            for line in self.browse(cr, 1, task_ids):
                if line.checking_task == 'none':
                    user_id = line.user_id.id
                elif line.checking_task == 'checking1':
                    user_id = line.previous_assigned_to1.id
        ...
        for employee in hr_employee_obj.browse(cr, 1, employee_ids):
            hr_sign_in = []
            hr_sign_out = []
            hr_attendance_ids = hr_attendance_obj.search(cr, 1, [('day','=',date_now),('employee_id','=',employee.id)])
            if hr_attendance_ids:
                for line in hr_attendance_obj.browse(cr, 1, hr_attendance_ids):
                    if line.action=='sign_in':
    ...
    def logout_all_attendance(self, cr, uid, context=None):
        ...
        for employee in hr_employee_obj.browse(cr, 1, employee_ids):
            ...
            if hr_attendance_ids:
                line = hr_attendance_obj.browse(cr, 1, hr_attendance_ids[0])
                if line.action=='sign_in':
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        stage_ids = stage_obj.search(cr, uid, [('name','in',['Drawing','Checking','Ready for delivery'])])
        if 'stage_id' in vals and vals['stage_id'] not in stage_ids:
            vals.update({'past_stage_id':vals['stage_id']})
...
class project_work(osv.osv):
    ...
    def _get_day(self, cr, uid, ids, name, arg, context=None):
        result = {}
        for line in self.browse(cr, uid, ids, context=context):
            result[line.id] = line.date[:10]
        return result
    ...
    _columns = {
        ...
        'day': fields.function(_get_day,type='char',store=True,string='Day'),
    }
project_work()
