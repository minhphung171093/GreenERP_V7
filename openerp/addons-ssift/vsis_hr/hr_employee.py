...
class hr_holidays(osv.osv):
    ...
    def _compute_number_of_days_temp(self, cr, uid, ids, name, args, context=None):
        result = {}
        for hol in self.browse(cr, uid, ids, context=context):
            DATETIME_FORMAT = "%Y-%m-%d %H:%M:%S"
            if hol.date_from and hol.date_to:
                from_dt = datetime.datetime.strptime(hol.date_from, DATETIME_FORMAT)
                from_dt = from_dt +  datetime.timedelta(hours=7)
                to_dt = datetime.datetime.strptime(hol.date_to, DATETIME_FORMAT)
                to_dt = to_dt +  datetime.timedelta(hours=7)
                timedelta = to_dt - from_dt
                if from_dt and (from_dt.hour < 8):
                    raise osv.except_osv(_('Warning!'),_('The start time must be greater than 08:00.')) 
                if to_dt and (to_dt.hour == 17):
                    if to_dt and (to_dt.minute > 48):
                        raise osv.except_osv(_('Warning!'),_('The end time must be less than 17:48.'))
                if from_dt.weekday() == 5 or from_dt.weekday() == 6:
                    diff_day = 0
                else:
                    diff = timedelta.days*24 + float(timedelta.seconds) / 3600
                    if from_dt.hour == 8 and to_dt.hour == 12:
                        diff_day = 4 
                    else:
                        if from_dt.hour<12 and to_dt.hour >= 13:
                            diff_day = diff - 1 < 8.8 and diff - 1 or 8.8 
                        else:
                            diff_day = diff < 8.8 and diff or 8.8 
                result[hol.id] = diff_day
        return result
    
    _columns = {
                'remaining_holiday': fields.float('Remaining Holiday', readonly=True, states={'draft':[('readonly',False)], 'confirm':[('readonly',False)]}),
                'number_of_days_temp': fields.function(_compute_number_of_days_temp,string='Allocation', type='float',store=True),
                }
...
# gio duoi database se cham hon tren he thong 7 tieng nen so lieu o duoi la co tru 7
        DATETIME_FORMAT = "%Y-%m-%d %H:%M:%S"
    ...
    def onchange_date_from(self, cr, uid, ids, date_to, date_from):
        ...
        """
        DATETIME_FORMAT = "%Y-%m-%d %H:%M:%S"
        start_time = datetime.datetime.strptime(date_from, DATETIME_FORMAT)
        start_time = start_time +  datetime.timedelta(hours=7)
        if date_from and (start_time.hour < 8):
            raise osv.except_osv(_('Warning!'),_('The start time must be greater than 08:00.'))
        result = {'value': {}}
        ...
        if date_from and not date_to:
            date_to_with_delta = datetime.datetime.strptime(date_from, tools.DEFAULT_SERVER_DATETIME_FORMAT) + datetime.timedelta(hours=8.8)
    ...
    def onchange_date_to(self, cr, uid, ids, date_to, date_from):
        ...
        """
        DATETIME_FORMAT = "%Y-%m-%d %H:%M:%S"
        end_time = datetime.datetime.strptime(date_to, DATETIME_FORMAT)
        end_time = end_time +  datetime.timedelta(hours=7)
        if date_to and (end_time.hour == 17):
            if date_to and (end_time.minute > 48):
                raise osv.except_osv(_('Warning!'),_('The end time must be less than 17:48.'))
        result = {'value': {}}
        if date_to and (end_time.hour > 17):
            raise osv.except_osv(_('Warning!'),_('The end time must be less than 17:48.'))
    ...
    def allocation_requests_auto_create(self, cr, uid, context=None):
        employee_ids = self.pool.get('hr.employee').search(cr, uid, [('active', '=', True)])
        sql =   '''               
                   select  id from hr_holidays_status where name like 'ANNUAL_LEAVE%' limit 1
                    '''
        cr.execute(sql)
        kq = cr.fetchall()
        for emp in employee_ids:
            for emp_id in self.pool.get('hr.employee').browse(cr, uid, [emp], context=context):
                val = {
                       'name': 'Allocation ' + str(datetime.datetime.today().month)+'/'+ str(datetime.datetime.today().year),
                       'state':'draft',
                       'user_id':emp_id.user_id.id,
                       'holiday_status_id': kq[0],
                       'employee_id':emp_id.id,
                       'number_of_days_temp': 8,
                       'type':'add',
                       'holiday_type':'employee',
                       }
                self.pool.get('hr.holidays').create(cr, uid, val, context=context)
        return True
