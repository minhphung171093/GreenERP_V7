            ...
            for line in xuly.chitiet_loai_xuly:
                ...
                for vacxin in self.pool.get('loai.vacxin').browse(cr,uid,vacxin_ids):
                    sql = '''
                        ...
                        where xuly_giasuc_id = %s and ct_loai_id = %s and vacxin_id = %s
                    '''%(xuly.id, line.ct_loai_id.id, vacxin.id)
                    cr.execute(sql)
                    sl_tp = cr.dictfetchone()['sl_tp']
                
                    if sl_tp>line.sl_tuong_duong:
                        raise osv.except_osv(_('Warning!'),_('Số lượng loài %s cùng với loại vaccine %s đã tiêm phòng không được lớn hơn số lượng loài %s muốn xử lý')%(line.name, vacxin.name,line.name))
                        return False
        return True
    ...
    def bt_duyet(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            for gs in line.chitiet_loai_xuly:
                sql = '''
                    select tiem_phong from chi_tiet_loai_vat where loai_id = %s and id = %s
